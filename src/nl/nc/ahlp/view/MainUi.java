...
import nl.nc.ahlp.controller.ILogControllerListener;
 ...
 *
 */
public class MainUi implements ILogControllerListener {
	...
	private void buildTableUI() {
		...
		table.addListener(SWT.SetData, new Listener() {
			public void handleEvent(Event e) {
				LogEntry entry = logController.readLine(e.index);
				
				if(entry!=null){
					TableItem item = (TableItem) e.item;
					item.setText(new String[] { "" + (e.index + 1), entry.getHostName(), entry.getDateAsString(),
							entry.getMethod(), entry.getRequest(), entry.getProtocol(), entry.getReponse() });
				}
			}
		});
	...
	/**
	 * Display directory selection dialog and load all access logs from that
	 * directory.
	 */
	private void selectLogFiles() {
		...
		if (selectLogs.open() != null) {
			if (selectLogs.getFileNames().length > 0) {
				List<File> logFiles = new ArrayList<File>();
				comboLogFiles.removeAll();
				comboLogFiles.add(new StringBuilder().append(selectLogs.getFileNames().length)
						.append(" file(s) selected.").toString());
				comboLogFiles.select(0);
				for (String fileName : selectLogs.getFileNames()) {
					comboLogFiles.add(new StringBuilder().append(selectLogs.getFilterPath()).append("\\")
				...
				try {
					...
					logController.loadLogs(logFiles);

					Display.getDefault().asyncExec(new Runnable() {

						@Override
						public void run() {
							table.setItemCount((int) logController.getItemCount());
						}
					...
					SwtUtil.showError(shlMain, "Error loading log files");
				}
			} else {
	...
	private void postLogsLoad(LogParser parser) {
		...
		// item.setText(new String[]{"" + (e.index + 1),
		// entry.getHostName(),entry.getDateAsString(),
		// entry.getMethod(),entry.getRequest(), entry.getProtocol(),
		// entry.getReponse()});
		// }
		// }
		// }
		...
		// tableContainer.setMinSize(table.computeSize(SWT.DEFAULT,
		// SWT.DEFAULT));
	...
	private void doShellResize() {
		...
		tableContainer.setBounds(stcX, stcY, shlMain.getClientArea().width - stcX,
				shlMain.getClientArea().height - stcY - lblState.getBounds().height);
		lblState.setBounds(lblState.getBounds().x, shlMain.getClientArea().height - lblState.getBounds().height,
				tableContainer.getBounds().width, lblState.getBounds().height);
	}
	...
	private void doFilter() {
		if (comboFilter.getSelectionIndex() > 0) {
			if (!StringUtil.isEmpty(textFilterValue.getText())) {
				...
				try {
					logController.filter(comboFilter.getItem(comboFilter.getSelectionIndex()),
							textFilterValue.getText(), checkRegExpr.getSelection());
	...
	private void doHelpRegex() {
		if (Desktop.isDesktopSupported()) {
			try {
				Desktop.getDesktop()
						.browse(new URI("http://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html"));
		...
		if (selectLogs.getFileName() != null && selectLogs.getFileName().length() > 0) {
			...
			try {
				if (logController.exportCsv(outputFile)) {
					SwtUtil.showInfo(shlMain, "Export to CSV",
							"Successfully exported to: " + outputFile.getAbsolutePath());
				}
			} catch (IOException e) {
	...
	public void update(Observable observable, Object result) {
		...
		if (updateResult != null && updateResult.size() > 0) {
			table.clearAll();
			table.setItemCount(updateResult.size());

			statusBuilder.append("Showing ").append(updateResult.size()).append(" of ")
					.append(updateResult.getTotalEntries());
		} else {
			table.removeAll();
	...
	@Override
	public void onLogSizeChange(long size) {
		this.table.setItemCount((int) size);
	}
}
